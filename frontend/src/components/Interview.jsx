...
//     if (currentAnswer && !feedback && !loading) {
//       handleSubmit()
...
//     // Strategy 1 — numbered lines like "1. Question" or "1) Question"
...
//           .replace(/^Why:.*$/i, '')
//           .trim()
//         if (q.length > 20 && q.includes('?')) parsed.push(q)
//       }
//     }

//     // Strategy 2 — split by double newline if strategy 1 failed
//     if (parsed.length === 0) {
//       const blocks = raw.split(/\n\n+/).filter(b => b.trim())
//       for (const block of blocks) {
//         const q = block.replace(/^\d+[.)]\s+/, '').replace(/\*\*/g, '').trim()
//         if (q.length > 20 && q.includes('?')) parsed.push(q)
//       }
//     }

//     // Strategy 3 — split by question marks if still nothing
//     if (parsed.length === 0) {
//       const sentences = raw.split('?').filter(s => s.trim().length > 20)
//       for (const s of sentences) {
//         parsed.push(s.trim().replace(/\*\*/g, '') + '?')
//       }
//     }
